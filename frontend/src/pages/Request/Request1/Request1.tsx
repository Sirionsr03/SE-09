...
import "./Request1.css"
import { FacultyInterface } from '../../../interfaces/Faculty';
import { GetDegrees, GetFaculty, GetMajorByFacultyID, GetStudentByID } from '../../../services/https';
import { MajorInterface } from '../../../interfaces/Major';
import { useLocation, useNavigate } from 'react-router-dom';
import { DegreesInterface } from '../../../interfaces/Degree';
...
const Request1: React.FC = () => {
	...
	const [degree, setdegree] = useState<DegreesInterface[]>([]);
	const [selectDegree, setSelectDegree] = useState("");
	...
	async function Summit() {
		...
		navigate('/request', { state: { datapdf: data1 } });
	...
	const getdegree = async () => {
		...
		if (res && res.data && Array.isArray(res.data)) {
			setdegree(res.data); // Assuming the array is under `res.data`
		} else {
								...
								<select name="format"
									...
									<option value="" disabled selected>กรุณาเลือกสาขา</option>
									{
										major.map((item, index) => {
											console.log(item)
											return (
												<option key={index} value={item.ID}>{item.MajorName}</option>
											)
										})
									}
								</select>
